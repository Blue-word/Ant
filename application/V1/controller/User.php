...
class User extends Common{
    ...
    public function imgsupload(){
        ...
        $rootpath = 'forum_image'; //根目录文件夹
        $savepath = 'seek'; //子目录文件夹
        $files = request()->file('image');
        foreach($files as $k => $v){
            //dump($file);
            // 移动到框架应用根目录/public/uploads/ 目录下
            $info = $v->move(ROOT_PATH . 'public' . DS . 'upload' . DS . $rootpath . DS . $savepath);
            if($info){
    ...
    public function publish(){
        ...
        if ($type) {
            ...
            if ($info['code']) {
                ...
                // $this->apiReturn("图片上传失败","400",$info);
            }
            print_r($info);
            dump($info);
        }
    ...
    public function is_collection(){
        ...
        $where['uid'] = $uid;
        $where['place'] = $place;
        $res = M('user_collection')->where($where)->count();
    ...
    public function userRelated(){
        ...
        if (empty($userInfo)) {
            $this->apiReturn("这里没有任何东西喔~","200","这里没有任何东西喔~");
        }
    ...
    public function postReply(){
        ...
        $data = array(
            ...
            'msg_id' => $uid,               //评论人
            'target_msg_id' => $target_msg_id, //评论的目标用户
