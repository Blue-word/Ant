...
class Ad extends Base{
    ...
    public function ad(){       
        ...
        if($ad_id){
            ...
            if ($ad_info['cate'] == '2') {
                $ad_info['picture'] = img_url_transform($ad_info['content'],'absolute');
            }
        }
    ...
    public function adHandle(){
        ...
        if ($data['cate'] == '2') {
            $preg = "/<img([^>]*)\s*src=('|\")(.*?)('|\").*?(\/>)/"; //正则表达式
            preg_match_all($preg,$data['content'],$matches,PREG_PATTERN_ORDER);
            $pic_url = $matches[3];  //匹配上传的图片
            $data['picture'] = implode('',$pic_url);
        }
